...
public class server {
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			//System.out.println("DEBUG: " + localServerName + " at IP address " + serverSocket.getInetAddress().getLocalHost().getHostAddress() + " and port number: " + serverSocket.getLocalPort());
		} catch (IOException e) {
		...
		try {
			...
			System.out.println("DEBUG: local info file [" + inFileName + "] does not exist");
				...
				if(localServerName.equals(serverName)&&(serverSocket.getLocalPort()!=serverPort)){
					...
					while(in.hasNext()) {
						...
						if(cmd.equalsIgnoreCase("y")) {
							...
							System.out.println("DEBUG: " + serverName + " restarted");
							break;
						}
							...
							try {
								...
								System.out.println("cannot delete local info files: "+inFileName);
								e.printStackTrace();
							}
						}
		...
		if(svrCrashed == true) {
			...
			//recoverFrFile(svrData, localServerName);//need to recover the backup data
			...
			for(int i = 0; i < servers.size(); i++) {
				if(!localServerName.equals(servers.get(i).getName())) {
					try {
						...
						System.out.println("DEBUG: " + localServerName + " " + "restart" + " " + serverSocket.getLocalPort() + " " + index);
						...
						System.out.println("from " + servers.get(i).getName() + ": " + ackMsg);
	...
	private static void recoverBkp(ServerData svrData, Scanner inSvr) {
		...
		System.out.println("DEBUG "+line);//print out the data&info message for debug purpose
	...
	private static void recoverFrFile(ServerData svrData, String localServerName) {
		Scanner inData = null;
		String localDir = "/tmp/92476/stream";
		//String localDir = "/Users/gladet/csc502/stream";
		String inFileName = localDir+"/"+localServerName+"_data"; //the local file to store the servers info, using absolute path
		try {
			...
			System.out.println("DEBUG: local data file [" + inFileName + "] does not exist, cannot recover data");
		}
		...
		try {
			...
			System.out.println("DEBUG: local info file [" + inFileName + "] does not exist, cannot recover info");
		}
	}
}
